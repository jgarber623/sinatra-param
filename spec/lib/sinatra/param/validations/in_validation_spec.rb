describe Sinatra::Param::InValidation do
  context 'when given a non-Array in value' do
    it 'raises an ArgumentError' do
      message = 'in must be an Array (given String)'

      expect { described_class.apply(:foo, 'bar', :string, in: 'biz') }.to raise_error(Sinatra::Param::ArgumentError, message)
    end
  end
  ...
  context 'when given an Array in value' do
    ...
    context 'when parameter value is in specified values' do
      it 'returns nil' do
        expect(described_class.apply(:foo, 'bar', :string, in: %w[bar biz baz])).to be(nil)
      end
    end
  end
end
